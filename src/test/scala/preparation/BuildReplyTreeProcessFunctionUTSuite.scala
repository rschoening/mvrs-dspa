...
class BuildReplyTreeProcessFunctionUTSuite extends FlatSpec with Matchers {
  ...
  "reply resolver" must "create correct tree" in {
    ...
    val rawComments: List[RawCommentEvent] = List(
      RawCommentEvent(commentId = firstLevelCommentId, personId, creationDate = utils.toDateTime(1000), None, None, None, Some(postId), None, 0),
  ...
  "reply resolver" must "create correct tree if unordered" in {
    ...
    val rawComments: List[RawCommentEvent] = List(
      ...
      RawCommentEvent(commentId = firstLevelCommentId, personId, creationDate = utils.toDateTime(5000), None, None, None, Some(postId), None, 0),
    )
