...
object BuildCommentsHierarchyJob extends App {
  ...
  env.setParallelism(4) // TODO with multiple workers, the broadcast stream watermark lags VERY much behind - it seems to not advance at all - HOW ABOUT THE WATERMARK OF THE first-level COMMENTS? same thing, apparently, due to the file splits
  ...
  val rootedComments = postComments
    ...
    .process(new ProgressMonitorFunction[CommentEvent]("TREE", 1000)).name("tree_monitor")

  val droppedReplies = rootedComments
    .getSideOutput(outputTagDroppedReplies)
    .process(new ProgressMonitorFunction[CommentEvent]("DROPPED", 1000)).name("monitor_dropped")
...
class BuildReplyTreeProcessFunction(outputTagDroppedEvents: OutputTag[CommentEvent])
  ...
  override def processBroadcastElement(reply: CommentEvent,
    ...
    //    println(s"${getRuntimeContext.getIndexOfThisSubtask}: " +
    //      s"watermark: ${utils.formatTimestamp(watermark)} - " +
    //      s"reply: ${utils.formatTimestamp(reply.creationDate)} - " +
    //      s"diff: ${(reply.creationDate - watermark) / (1000f * 60f)} min")
